...
public class WikiNoteEditor extends BorderPane {
    ...
    private void setupListeners() {
        controller.addOnNoteChangedListener(Wiki -> {
            if (Wiki != null) {
                loadNote(Wiki);
            } else {
                showEmptyState();
    ...
    private HBox createNoteHeader() {
        ...
        exportMdItem.setOnAction(e -> {
            if (currentNote != null) {
                ExportUtils.exportNoteToMarkdown(currentNote, this.getScene().getWindow());
            }
        });
        ...
        deleteItem.setOnAction(e -> {
            if (currentNote != null) {
                ...
                alert.setContentText(currentNote.getTitle());
    ...
    private void renderMarkdown() {
        ...
        previewPane.getEngine().locationProperty().addListener((obs, oldLoc, newLoc) -> {
            if (newLoc != null && newLoc.startsWith("Wiki://")) {
                String noteTitle = newLoc.substring(7).replace("%20", " ");
                javafx.application.Platform.runLater(() -> {
                    controller.openNoteByTitle(noteTitle);
    ...
    public void loadNote(Wiki Wiki) {
        ...
        favoriteBtn.setOnAction(e -> {
            if (currentNote != null) {
                controller.toggleFavorite(currentNote);
                updateFavoriteButton();
            }
        });
    }
    ...
    /**
     * Loads notes for a specific operation. If notes exist for the operation,
     * loads the first one. If no notes exist, creates a new note for the operation.
     */
    public void loadNotesForOperation(Operation operation) {
        if (operation == null) {
            showEmptyState();
            return;
        }

        List<Wiki> operationNotes = controller.loadNotesForOperation(operation);
    ...
    public void handleExportMarkdownFromMenu() {
        if (currentNote != null) {
            com.roam.util.ExportUtils.exportNoteToMarkdown(currentNote, getScene().getWindow());
        }
    }
    ...
    private void updateStats() {
        if (currentNote != null) {
            ...
            if (currentNote.getUpdatedAt() != null) {
                String formatted = currentNote.getUpdatedAt().format(
    ...
    private void handleDuplicateWiki() {
        ...
        alert.setContentText("A duplicate will be created with the title '" + currentNote.getTitle() + " (Copy)'");
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                Wiki duplicate = controller.duplicateNote(currentNote);
                if (duplicate != null) {
                    loadNote(duplicate);
    ...
    private void handleAddBanner() {
        ...
        if (selectedFile != null) {
            ...
            currentNote.setBannerUrl(imageUrl);
            controller.scheduleAutoSave();
            updateBannerDisplay();
        }
    }
    ...
    private void handleRemoveBanner() {
        if (currentNote == null)
            return;
        currentNote.setBannerUrl(null);
        controller.scheduleAutoSave();
        updateBannerDisplay();
    }

    private void updateBannerDisplay() {
        if (currentNote != null && currentNote.getBannerUrl() != null && !currentNote.getBannerUrl().isEmpty()) {
            try {
                Image image = new Image(currentNote.getBannerUrl());
    ...
    private void createPropertiesSection() {
        ...
        regionCombo.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (currentNote != null && newVal != null) {
                currentNote.setRegion(newVal.getName()); // Storing region name for now as per Wiki model
        ...
        operationCombo.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (currentNote != null) {
                currentNote.setOperationId(newVal != null ? newVal.getId() : null);
                controller.saveCurrentNote();
            }
        });
        ...
        taskCombo.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (currentNote != null) {
                currentNote.setTaskId(newVal != null ? newVal.getId() : null);
                controller.saveCurrentNote();
            }
        });
        ...
        eventCombo.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (currentNote != null) {
                currentNote.setCalendarEventId(newVal != null ? newVal.getId() : null);
                controller.saveCurrentNote();
            }
        });
    }
